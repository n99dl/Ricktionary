...
public class DictionaryManagement {
    ...
    public void insertFromFile() {
        int i = -1;
        try (Scanner scanner = new Scanner(new File("src/main/java/sample/dictionaries_out.txt"))) {
...
//            System.out.println(wordsArray[1]);
    ...
    public void updateWord(String wordTarget, String newWordExplain){
        wordTarget = wordTarget.toLowerCase();
        ArrayList<Word> words = dictionaryLookup(wordTarget);
        newWordExplain = newWordExplain.replace("\n", "<br />");
        newWordExplain = newWordExplain.replace("'", "''");
        newWordExplain = newWordExplain.replace("\"", "\\\"");
        newWordExplain = newWordExplain.replace("\\", "\\\\");
        ...
        System.out.println(newWordExplain);
        data.update(wordTarget, "<C><F><I><N><Q>" + newWordExplain + "</Q></N></I></F></C>");
    }
